class histogram{
  ...
  histogram(ArrayList<flightAndDate> f,boolean val){
...
void histSetup(){
  ...
  if(!vertical) plot.startHistograms(GPlot.VERTICAL);
  else plot.startHistograms(GPlot.HORIZONTAL);
  GPointsArray points = new GPointsArray(size);
  
    if(this.flightsInRange!=null && !vertical){ 
        for (int i = 0; i < size; i++) {
          flightAndDate f = flightsInRange.get(i);
          points.add(i, f.number,f.getDate()); }
    }
    
      else  if(this.flightsInRange!=null && vertical){ 
        for (int i = 0; i < size; i++) {
          flightAndDate f = flightsInRange.get(i);
          points.add(f.number,i,f.getDate()); }
    }
